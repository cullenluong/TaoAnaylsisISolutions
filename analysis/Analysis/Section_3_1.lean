...
theorem SetTheory.Set.singleton_iff (A:Set) (hA: A ≠ ∅) : (¬∃ B ⊂ A, B ≠ ∅) ↔ ∃ x, A = {x} := by
  ...
  -- constructor
  -- · intro h
  --   --given that set A is empty, that for any set x that is a subset of A, A is empty
  --   --there exists an x such that A ={x}
  --   -- for any set x that is a strict subset of A, then x is empty
  --   --if A is not empty then there exists an object y that is a member of A
  --   -- singleton
  --   have ⟨x, hxA⟩ := nonempty_def hA
  --   let X: Set:={x}

  --   --
  --   have hye: {x} ⊂ A → x =empty :=by
  --     intro hxsA
  --     specialize h {x}
  --     simp[ssubset_def,subset_def] at h hxsA
  --     apply h at hxA
  --     obtain ⟨left,right⟩ := hxsA
  --     apply hxA at right
  --     simp[eq_empty_iff_forall_notMem] at right
  --   use x
  --   ext y
  --   simp[mem_singleton]
  --   constructor
  ...
  --     --simp[← mem_singleton]
  --     let Y: Set:={y}
  --     --simp[ssubset_def,subset_def,Set.ext_iff] at h

  --     have hy0: y = empty :=by
  --       specialize h {y}
  --       simp[ssubset_def,subset_def] at h
  --       apply h at hyA
